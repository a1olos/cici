...
//    @Before
    public void init() throws UnknownHostException {
        ...
        //获取movie DB；如果默认没有创建，mongodb会自动创建
        db = mg.getDB("th");
        //获取users DBCollection；如果默认没有创建，mongodb会自动创建
        users = db.getCollection("movie");
    }
...
//        print(users.save(user).getN());
        ...
        //添加多条数据，传递Array对象
        wr =  users.insert(user, new BasicDBObject("name", "tom"));
...
//    @Test
    public void testOthers() {
        ...
        print("判断movie Collection是否存在: " + db.collectionExists("movie"));
        
        //如果不存在就创建
        if (!db.collectionExists("movie")) {
